...
<!--<table id='Welcomepage'  style="width:100%;" cellpadding="0" cellspacing="0">
	...
	<div id="MainDiv" class="Sticky">
		<div id="div_Load"  class="cEngView" style='margin-bottom: 0px; margin-top: 0px !important;'>
		...
		<div  class="row  " id='cEngNewLenrow' style="margin-top:0px;width:fit-content">                    	 
			...
			<span class ='cEngNewLen' id="resorceList" style="float: left;">
		...
		<div><span id="addItemChk" style="color: red;"></span></div>
	</div>
...
function resorceDrop(){
	...
	drpResID.multiselect({
	  ...
	  allSelectedText: 'All Resource',
	  templates: {
        filter: '<li class="multiselect-item filter"><input type="button" id="btnReset" value="Reset" /><div class="input-group"><input class="form-control multiselect-search" type="text"></div></li>',
      },
	  onChange: function(e){
...
if($('input[title="Project Name"]').length ==0){			
 ...
 /*  globalStrtDt = val1;
  ...
  var projectcode = getParameterByName('projuid');
   ...
   var url = azurewebsites+"/api/AllocationTimePhasewithEmpty/"+projectcode+'/'+startDate+'/'+EndDate//azurewebsites+"/api/Allocation/"+projectcode+'/'+startDate+'/'+EndDate
  ...
  function createResourceFilterdropdowns(){
    ...
    if(ResFilter.length > 0){
	  ...
	  $('#typeIsNamed').multiselect({
		...
		nSelectedText: 'Type',
		allSelectedText: 'All Type',
		templates: {
          filter: '<li class="multiselect-item filter"><input type="button" id="typeReset" value="Clear" /><div class="input-group"><input class="form-control multiselect-search" type="text"></div></li>',
        },
		...
		if(counter < 4){
		  ...
		  if(resFilter_ar.length > 0){
			...
			var optional = $("#optional"+['Filter'+po]).multiselect({
		      ...
		      allSelectedText: 'All '+MGrid.ProjectFilter,
			  onChange: function(e){
				global_onchangeEle = ""
				global_onchangeEle = this.$select[0].id;
			    var drp_id = this.$select[0].id;
				var col_nm = drp_id.replace('optional','');
				changedrop = this.$select[0].id;
				if($("#"+drp_id).val() != null){
				  var cc = $("#"+drp_id).val().length;
				  $("#"+global_onchangeEle).prev().find('li').text(MGrid.ProjectFilter+ " ("+cc+")")
				  $("#"+drp_id).prev().find('span').show();
				}else{ 	
				  $("#"+drp_id).prev().find('input').val(MGrid.ProjectFilter);
			    } 
				var multiar = [];
				filterApplied =  true;
			    var item = e.item;
				...
				lastselected = MGrid.ProjectFilter.replace(/\s+/g, '');
	...
	$("#optional"+['Filter'+po]).multiselect('dataprovider', finalArr);
								...
								/*var dropdownlistF = $("#optional"+['Filter'+po]).data("kendoMultiSelect");
	...
	$("#optionalFilter"+po).multiselect('dataprovider', finalArr);
								//var dropdownlistF = $("#optionalFilter"+po).data("kendoMultiSelect");
								//dropdownlistF.setDataSource(arr);
...
//multiselect.setDataSource(Res_data);
  ...
  function setotherdropdown(Secman,fut,mft,multiar){
	...
	var checkbox_li = $(this).find('select').data("multiselect");
			...
			if(ity_len > 0){
				for(k=0;k<ity_len;k++){
					  if(item[k][nm] == dropval[j]){
						...
						//dropval_new.push({"label":dropval[j],"value":dropval[j]})
		...
		//$(this).find('select').multiselect('dataprovider', dropval_new);//data("kendoMultiSelect").value(dropval_new);
